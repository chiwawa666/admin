  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="死亡时间" align="center">
        <template slot-scope="{ row }">
          <span class="link-type">{{ row.deathTime }}</span>
        </template>
      </el-table-column>
      <el-table-column label="家属姓名" align="center">
        <template slot-scope="{ row }">
          <span class="link-type">{{ row.familyName }}</span>
        </template>
      </el-table-column>
      <!-- <el-table-column label="逝者关系" align="center">
        <template slot-scope="{ row }">
          <span class="link-type">{{ row.familyRelation }}</span>
        </template>
      </el-table-column> -->
      <el-table-column label="联系方式" align="center">
        <template slot-scope="{ row }">
          <span class="link-type">{{ row.familyPhone }}</span>
        </template>
      </el-table-column>
      <el-table-column
        label="操作"
      ...
      >
        <template slot-scope="{ row }">
          ...
          <router-link :to="'/departed/edit/' + row.id">
          ...
          <router-link
            :to="
              '/departed/service/' +
              (row.boneashRecord !== null
                ? row.boneashRecord.defunctid
                : row.id)
            "
            v-if="row.status == 2 || row.status == 3"
          >
            <el-button type="info" size="mini"> 选择服务 </el-button>
          </router-link>
        </template>
      </el-table-column>
    </el-table>
  ...
  methods: {
    getList() {
      ...
      getPeopleList(this.listQuery).then((response) => {
        ...
        this.list = JSON.parse(response.result).rows;
        this.total = JSON.parse(response.result).total;
