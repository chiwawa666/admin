<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="使用产品" align="center">
        <template slot-scope="{ row }">
          <span>{{ row | productFilter }}</span>
        </template>
      </el-table-column>
      ...
      >
        ...
        >
          <el-button type="primary" size="mini" @click="stateChange(row)">
...
export default {
  ...
  filters: {
    productFilter(row) {
      if (row.productInfoVos) {
        return row.productInfoVos.map((item) => item.names).join(",");
      }
    },
    stateFilter(state) {
      const stateMap = {
  ...
  methods: {
    getList() {
      ...
      getThird(this.listQuery).then((response) => {
    ...
    stateChange(row) {
      ...
      const stateCode = {
        //这里的状态是下一阶段的状态 例:待受理 0->1
        待受理: "1",
        已受理: "2",
      };
      const state = stateCode[stateStr];
      const { orderno } = row;
      console.log("row", row);
      updateThirdState({ orderno, state }).then((response) => {
    ...
    handleUpdate(row) {
      ...
      getThirdInfo(row.orderno).then((response) => {
